    ...
    def add_undulation(self, scale=0.00002, amplitude=2000000, octaves=4, seed=0):
        ...
        for y in range(self.image_size):
            for x in range(self.image_size):
                nx = x * scale
                ny = y * scale
                noise_val = pnoise2(nx, ny, octaves=octaves, repeatx=1024, repeaty=1024, base=seed)
                elevation_noise[y, x] = noise_val

        # Use raw noise values in range [-1, 1] and scale
        self.elevation += elevation_noise * amplitude
        # Print the max and min value in the elevation matrix
        print(
            f"Undulation Elevation Map Range: {self.elevation.min():.2f}m to {self.elevation.max():.2f}m | Δ={self.elevation.max() - self.elevation.min():.2f}m"
        )
